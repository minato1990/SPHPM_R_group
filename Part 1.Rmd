---
title: "Introduction to R -  Week 1"
...
```{r }
# create ses variable
ses <- c("low", "middle", "low", "high", "low", "low", "low", "middle", "low",
...
```{r}
ses.f <- factor(ses, levels = c("middle","low", "high"))
ses.f
levels(ses.f)
# or
ses.f <-factor(ses.f,levels(ses.f)[c(2,1,3)])
ses.f
levels(ses.f)
```

We can also generate factor variable from numeric data by specifing labels using the labels argument. 

<span style="color:Green;font-weight: bold;font-size:1.2em ">`r Challenge_nums(name="factor label",caption = "")`</span>  Crate a factor variable for type as type.f and label 0 as "private" and 1 as "public".
...
# your code: 
type.f<-factor(type, labels=c("private", "public"))
type.f
...
```{r}
factor_to_numeric = c(10,20,20,50,10,20,10,50,20)
factor_to_numeric = factor(factor_to_numeric,levels=c(10,20,50))
factor_to_numeric=="20"
```
...
```{r}
mean(as.numeric(factor_to_numeric))
mean(as.numeric(levels(factor_to_numeric)[factor_to_numeric]))
mean(as.numeric(as.character(factor_to_numeric)))
```
#### Ordered factor
...
```{r,  echo=FALSE, results="hide"}
levels(ses.f)[levels(ses.f)=='very.high'] <- NA
